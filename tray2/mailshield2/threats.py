...
_BTC_LEGACY_RE = re.compile(r"(?<![A-Za-z0-9])[13][a-km-zA-HJ-NP-Z1-9]{25,34}(?![A-Za-z0-9])")
_BTC_BECH32_RE = re.compile(r"(?<![A-Za-z0-9])bc1[ac-hj-np-z02-9]{25,62}(?![A-Za-z0-9])")
_ETH_RE = re.compile(r"(?<![A-Za-z0-9])0x[a-fA-F0-9]{40}(?![A-Za-z0-9])")
_EXTORTION_SIGNALS: tuple[tuple[str, re.Pattern[str]], ...] = (
    ("malware", re.compile(r"(?i)(malware|trojan|spyware|rat\b|remote\s+access|해킹|악성\s*코드|원격\s*제어|스파이웨어|감염)")),
    ("webcam", re.compile(r"(?i)(webcam|camera|recorded|video\s+of\s+you|masturbat|adult\s+(?:site|content)|웹캠|카메라|녹화|영상|음란|성인\s*사이트)")),
    ("deadline", re.compile(r"(?i)(\b\d{1,3}\s*(?:hours|hrs|days)\b|\d{1,3}\s*(?:시간|일)\s*(?:이내|안에|내에))")),
    ("threat", re.compile(r"(?i)(send\s+(?:it|this|the\s+video)\s+to\s+(?:all|your)|contacts|friends\s+and\s+family|publish|leak|spread|지인|연락처|유포|공개|퍼뜨|친구|가족)")),
    ("payment", re.compile(r"(?i)(bitcoin|btc\b|wallet|monero|ethereum|비트코인|지갑|송금|입금|payment|transfer|\$\s?\d{3,}|\d{3,}\s?(?:usd|달러))")),
)


def _base58_checksum_ok(address: str) -> bool:
    value = 0
    for char in address:
    ...
    raw = value.to_bytes(25, "big") if value < (1 << 200) else b""
    if len(raw) != 25:
        return False
...
def find_crypto_wallets(text: str) -> list[str]:
    """비트코인(레거시 주소는 체크섬 검증)·bech32·이더리움 주소를 찾는다. 값은 돌려주지만 저장하지 않는다."""
    found = [m.group() for m in _BTC_LEGACY_RE.finditer(text) if _base58_checksum_ok(m.group())]
    found += [m.group() for m in _BTC_BECH32_RE.finditer(text)]
    found += [m.group() for m in _ETH_RE.finditer(text)]
    return found


@dataclass
class ExtortionReport:
    findings: list[str] = field(default_factory=list)
    risk: str = RISK_SAFE
    wallets: int = 0
...
def scan_extortion_text(text: str) -> ExtortionReport:
    """성착취·폭탄 등 협박 메일: 암호화폐 지갑 + 협박 지표가 함께 있을 때만 판정한다."""
    report = ExtortionReport()
    report.wallets = len(find_crypto_wallets(text))
    report.signals = [key for key, pattern in _EXTORTION_SIGNALS if pattern.search(text)]
    strong = {"malware", "webcam", "threat"} & set(report.signals)
    if report.wallets and len(report.signals) >= 3 and strong:
        report.findings.append(f"협박 메일 의심(암호화폐 지갑 {report.wallets}건 + " + ", ".join(report.signals) + ")")
        report.risk = RISK_HIGH
    elif report.wallets and len(report.signals) >= 2:
        report.findings.append(f"암호화폐 지갑 {report.wallets}건과 협박 문구")
        report.risk = RISK_MEDIUM
    return report
