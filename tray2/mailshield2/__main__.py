"""진입점: python -m mailshield2  또는 MailShield2Tray.exe
...
def _verify(gui: bool) -> int:
    ...
    log = logging.getLogger("mailshield2.verify")
    settings = Settings.load()
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="mailshield2", description=APP_DISPLAY_NAME)
    ...
    log = logging.getLogger("mailshield2")
    ...
    if not instance.acquire():
        ...
        try:
            ...
            root = tk.Tk()
            root.withdraw()
            messagebox.showinfo(APP_DISPLAY_NAME, "MailShield 2가 이미 실행 중입니다. 작업 표시줄 트레이 아이콘을 확인하십시오.")
            root.destroy()
        except Exception:
            pass
        return 0
