...
class ScanResult:
    ...
    categories: list[str] = field(default_factory=list)  # pii | link | sender | attachment | ransom | extortion
...
# 인증정보 키워드 옆에 값처럼 보이는 문자열이 붙어 있는지(= 실제 유출인지) 본다.
_CREDENTIAL_VALUE_RE = re.compile(
    r"(?i)(?:password|passwd|pwd|otp|api[_ -]?key|secret[_ -]?key|access[_ -]?token|비밀번호|패스워드|인증번호)"
    r"\s*(?:는|은|이|가)?\s*[:=]?\s*[\"'\[(]?([A-Za-z0-9!@#$%^&*_+\-./]{6,})"
)


def credential_has_value(text: str) -> bool:
    """'비밀번호: abc123!' 처럼 값이 이어질 때만 True. 'API key를 발급받으세요'는 False."""
    for match in _CREDENTIAL_VALUE_RE.finditer(text):
        token = match.group(1)
        if token.lower() in ("발급", "재발급", "입력", "변경", "확인", "안내", "재설정", "설정"):
            continue
        if any(c.isdigit() for c in token) or len(token) >= 10:
            return True
    return False


def _risk_from_counts(counts: dict[str, int], credential_value: bool = True) -> str:
    high_keys = [key for key in counts if key in _RULE_BY_KEY and _RULE_BY_KEY[key].risk == RISK_HIGH]
    if not credential_value:
        high_keys = [key for key in high_keys if key != "credential"]
    if high_keys:
        return RISK_HIGH
    if counts:
        return RISK_MEDIUM
...
def scan_message(raw_message: bytes, folder: str, uid: int, own_addresses: Iterable[str] = ()) -> ScanResult:
    ...
    categories: list[str] = ["pii"] if counts else []

    # 랜섬웨어 협박 문구·확장자 언급
    ransom = threats.scan_ransomware_text(text)
    findings.extend(ransom.findings)
    if ransom.findings:
        categories.append("ransom")

    # 협박(성착취·폭탄) 메일: 암호화폐 지갑 + 협박 지표
    extortion = threats.scan_extortion_text(text)
    findings.extend(extortion.findings)
    if extortion.findings:
        categories.append("extortion")

    # 발신자 검증(인증 헤더·표시명 위장·기관 사칭·가짜 스레드)
    sender_report = sender_module.analyze_sender(message, tuple(own_addresses))
    findings.extend(sender_report.findings)
    if sender_report.findings:
        categories.append("sender")
    counts.update({f"sender_{k}": v for k, v in sender_report.counts.items()})

    # 링크 검사(표시·실제 불일치, 단축·리다이렉트, 퓨니코드, 숨긴 링크, PDF 안 링크)
    link_report = links_module.analyze_message_links(message, sender_report.from_domain)
    findings.extend(link_report.findings)
    if link_report.findings:
        categories.append("link")
    counts.update({f"link_{k}": v for k, v in link_report.counts.items()})
    ...
    delivery = threats.ransomware_delivery_findings(reports)
    findings.extend(delivery)
    if attachment_risk != RISK_SAFE or delivery:
        categories.append("attachment")

    risk = _max_risk(_risk_from_counts(counts, credential_has_value(text)), ransom.risk, extortion.risk, sender_report.risk, link_report.risk, attachment_risk)
    return ScanResult(
        folder=folder,
        uid=uid,
