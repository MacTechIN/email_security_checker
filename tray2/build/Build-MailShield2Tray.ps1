<#
.SYNOPSIS
  MailShield 2를 단일 exe로 빌드하고 Inno Setup 설치기를 만든다.

.DESCRIPTION
  1. 단위 테스트 실행 (-SkipTests로 생략)
  2. 아이콘 생성
  3. PyInstaller --onefile --noconsole → dist\MailShield2Tray.exe
  4. Inno Setup(ISCC.exe) → output\MailShield2Tray-Setup.exe (-SkipInstaller로 생략)
...
Write-Host "== MailShield 2 빌드 ($TrayRoot)" -ForegroundColor Cyan
...
Write-Host "-- 아이콘 생성"
python build\make_icon.py build\MailShield2Tray.ico

Write-Host "-- PyInstaller"
if (Test-Path dist) { Remove-Item dist -Recurse -Force }
if (Test-Path build\pyi) { Remove-Item build\pyi -Recurse -Force }
python -m PyInstaller --clean --noconfirm --distpath dist --workpath build\pyi build\MailShield2Tray.spec
if ($LASTEXITCODE -ne 0) { throw "PyInstaller 실패" }

$exe = Join-Path $TrayRoot "dist\MailShield2Tray.exe"
...
if (-not $SkipInstaller) {
    ...
    if (-not $iscc) {
        ...
        & $iscc /Qp "build\MailShield2Tray.iss"
        if ($LASTEXITCODE -ne 0) { throw "Inno Setup 실패" }
        Get-ChildItem output\MailShield2Tray-Setup*.exe | ForEach-Object {
