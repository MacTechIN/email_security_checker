...
from mailshield2 import scanner
...
def test_credential_keyword_without_value_is_medium():
    """v2: 단순 언급은 '주의'. 'API key를 발급받으세요' 같은 뉴스레터 오탐을 낮춘다."""
    result = _scan("본문에는 아무것도 없음", subject="비밀번호 안내")
    assert "인증정보 키워드" in result.findings
    assert result.risk == "medium"


def test_credential_keyword_with_value_is_high():
    result = _scan("로그인 정보입니다. 비밀번호: Str0ngPass!")
    assert "인증정보 키워드" in result.findings
    assert result.risk == "high"
