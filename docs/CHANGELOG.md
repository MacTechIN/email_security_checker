...
| 버전 | 날짜 | 유형 | 설명 |
|---|---|---|---|
| 0.1.12 | 2026-09-11 | build | **배포용 패키징을 Nuitka 기계어 컴파일로 전환.** 설치기(`MailShieldTray-Setup-0.1.11.exe`, 28.8 MB)에 `.py`/`.pyc`가 0개임을 무인 설치·검사로 확인, 설치본 `--verify` 종료 코드 0. 프로세스 1개·기동 1.1초·임시 폴더 없음(PyInstaller onefile은 2개·10초·106 MB 추출). 폴더형 설치기 정의 `MailShieldTray-dir.iss` 추가, 시작 메뉴에 '연결 점검' 항목. 잔여 노출: 모듈·일부 함수 이름과 규칙 문자열 일부는 바이너리에 평문으로 남음(로직 구조는 복원 불가). `docs/07`·`docs/08` 갱신 |
| 0.1.11 | 2026-09-11 | feat | **처음 설정 마법사와 `--verify` 점검 모드.** 설치기가 설치 직후 마법사를 연다. 이메일 입력 → 해당 계정의 2단계 인증·앱 비밀번호 페이지를 열어 주고 실제 화면 기준 단계를 옆에 표시 → 완료 체크 → 붙여넣기(16자리 형식 검사) → 자동 연결 검사 → 통과 시에만 설정 저장·자동 시작 등록·감시 시작. `--verify [--gui]`는 트레이 없이 설정·자격증명·IMAP 연결을 점검하고 종료 코드(0/1/2)로 알린다. 버전 표기 0.1.10으로 정정. onedir spec 추가(프로세스 1개 검증용). `docs/08.배포안내.md` 신설. 테스트 82개 |
| 0.1.10 | 2026-09-11 | feat | **근거 창에 '당시 판정 재현' 추가.** 지금 규칙으로 근거가 없을 때 0.1.6 이전 규칙(링크 포함, 영문 키워드 단어 경계 없음)으로 다시 검사해 무엇이 어디서 잡혔는지 [링크 안] 표시와 함께 보여 준다. 저장하지 않는다. 함께: `docs/07.경량화계획서.md` 신설(프로세스·메모리·디스크 실측, 개선안 시뮬레이션, 진행 순서, 확인 요청). 테스트 79개 |
