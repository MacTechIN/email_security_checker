...
`MAILSHIELD_AGENT_API_KEY`를 설정하면 heartbeat, 장치 조회와 정책 API가 `X-MailShield-Agent-Key` 헤더를 요구합니다. 개발 편의상 키가 비어 있으면 인증을 생략하지만 운영 환경에서는 반드시 길고 무작위인 비밀값을 설정해야 합니다.
